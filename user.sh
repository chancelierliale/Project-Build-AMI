...
sudo useradd -m -s /bin/bash -G sudo master-user
echo -e 'master-user  ALL=(ALL) NOPASSWD: ALL' | sudo tee /etc/sudoers.d/master-user
sudo -u master-user ssh-keygen -t rsa -N '' -f /home/master-user/.ssh/id_rsa  
sudo cp /home/master-user/.ssh/id_rsa.pub /home/master-user/.ssh/authorized_keys
